...
public class OdinAOTBuilder : IPreprocessBuildWithReport, IPostprocessBuildWithReport
{
    ...
    public static void GenerateSerializedTypesDLL()
    {
        ...
        DependentTypesFinder.FindDependentTypes(typeof(SaveData), ref serializedTypes);
...
{
    private static readonly Type[] _excludedTypes = { typeof(Action), typeof(Action<>), typeof(Action<,>), typeof(Action<,,>), typeof(Action<,,,>), typeof(void*), typeof(Delegate), typeof(IntPtr) , typeof(UnityEngine.Object) };
    ...
    public static void FindDependentTypes(Type type, ref List<Type> resultTypes)
    {
        if (resultTypes.Contains(type) || _excludedTypes.Contains(type)
            || IsSubclassExcludedTypes(type) || type.Name.Contains("Action"))
            return;
    ...
    {
        if (!types.Contains(type) && !_excludedTypes.Contains(type) && !IsSubclassExcludedTypes(type))
            types.Add(type);
    }
    ...
    private static bool IsSubclassExcludedTypes(Type type)
    {
        foreach (Type excludedType in _excludedTypes)
